...
searchDataTable = () => {
    ...
    var table = $("#datatable-default-").DataTable({
        ...
        ordering: true, 
        ajax: {
            url: getAPIUrl() + "Area/GetForDataTable",
        ...
        columns: [
            ...
            },
            {
                "data": "active", render: function (data, type, row) {
                    return addCheckedActionGuid('Area/ToggleActive', row);
                }
            },
            ...
            { "data": "formattedModifiedOn", render: function (data, type, row) { return row.formattedModifiedOn; } },
            {
                "data": null, "name": "Actions", render: function (data, type, row) { 
                    return `${addDropDownMenuOptions("Area", "/Settings/Area/", row, { edit: allowEdit, delete: allowDelete })}`;
                }
            }, 
        ],
...
cbPostSuccess = (data) => { 
    if (data.success) {
        ...
        setTimeout(() => location.href = "/Settings/Area/AreaList", 1000);
    } else {
        ToastAlert('error', 'Area', data.message);
    }
}
